...
public class ProdutoController : ControllerBase{
    ...
    [HttpPost]
    [Route("Cadastrar")]
    public async Task<ActionResult> Cadastrar(Produto produto, int categoriaID)
    {
        if (_dbContext is null) return NotFound();
        if (_dbContext.Categoria is null) return NotFound();

        var categoriaTemp = await _dbContext.Categoria.FirstOrDefaultAsync(c => c.Id == categoriaID);
